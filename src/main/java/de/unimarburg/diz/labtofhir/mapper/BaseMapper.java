...
abstract class BaseMapper<T> implements ValueMapper<T, Bundle> {
    ...
    public static final String SYSTEM_V_2_0074 =
            "http://terminology.hl7.org/CodeSystem/v2-0074";
    private static final int MAX_IDENTIFIER_LENGTH = 64;
    private final IParser fhirParser;
    private final FhirProperties fhirProperties;
    private final Function<String, String> hasher =
            i -> Hashing.sha256().hashString(i, StandardCharsets.UTF_8)
                    .toString();
    ...
    BaseMapper(FhirContext fhirContext, FhirProperties fhirProperties,
        ...
        identifierAssigner = new Identifier().setSystem(
                        fhirProperties.getSystems().getAssignerId())
    ...
    protected List<CodeableConcept> getReportCategory() {
        return List.of(new CodeableConcept().addCoding(
                        new Coding().setSystem("http://loinc.org").setCode("26436-6"))
                .addCoding(new Coding().setSystem(SYSTEM_V_2_0074)
                        .setCode("LAB")));
    }
    ...
    protected boolean createDateTimeFilter(DateTimeType date,
        ...
        var targetDate =
                date.getValue().toInstant().atZone(ZoneId.systemDefault())
                        .toLocalDate();
    ...
    private CodeableConcept initServiceRequestCode() {
        return new CodeableConcept().setCoding(
                List.of(new Coding().setSystem("http://snomed.info/sct")
                        .setCode("59615004")));
    }
    ...
    protected Reference getObservationsReference(Observation obs) {
        return new Reference(getConditionalReference(ResourceType.Observation,
    ...
    protected void addResourceToBundle(Bundle bundle, DomainResource resource,
        ...
        bundle.addEntry().setResource(resource).setRequest(
                new Bundle.BundleEntryRequestComponent().setMethod(
                        ...
                        resource.getResourceType(), identifier)));
    }
    ...
    protected void setPatient(String patientId, Bundle bundle) {
        ...
        getBundleEntryResources(bundle, ServiceRequest.class).forEach(
                r -> r.getSubject().setReference(
                        getConditionalReference(ResourceType.Patient,
                                patientId)));
    ...
    protected void setEncounter(String encounterId, Bundle bundle) {
        getBundleEntryResources(bundle, ServiceRequest.class).forEach(
                r -> r.getEncounter().setReference(
                        getConditionalReference(ResourceType.Encounter,
                                encounterId)));
        getBundleEntryResources(bundle, DiagnosticReport.class).forEach(
                r -> r.getEncounter().setReference(
                        getConditionalReference(ResourceType.Encounter,
    ...
    protected Quantity parseQuantity(String value) {
        ...
        if ((comp == '<' || comp == '>')
                && NumberUtils.isCreatable(valuePart)) {
            return new Quantity(
