...
abstract class BaseMapper<T> implements ValueMapper<T, Bundle> {
  ...
  BaseMapper(FhirContext fhirContext, FhirProperties fhirProperties, DateFilter dateFilter) {
    ...
    this.mapperTag =
        new Coding()
            .setSystem(fhirProperties().getSystems().getMapperTagSystem())
  ...
  protected List<CodeableConcept> getReportCategory() {
    return List.of(
        new CodeableConcept()
            .addCoding(new Coding().setSystem("http://loinc.org").setCode("26436-6"))
            .addCoding(
                new Coding()
  ...
  protected Meta getMeta(String resourceType) {
    ...
    if (profile == null)
      throw new IllegalStateException(
          String.format("FHIR Profile configuration for %s is missing!", resourceType));

    return new Meta().addProfile(profile).addTag(getMapperTag()).setSource("#swisslab");
  }
  ...
  protected boolean createDateTimeFilter(DateTimeType date, DateFilter filter) {

    var targetDate = date.getValue().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
    var threshold = filter.date();

    return switch (filter.comparator()) {
      case "<" -> targetDate.isBefore(threshold);
      case "<=" -> targetDate.isBefore(threshold) || targetDate.isEqual(threshold);
      case ">" -> targetDate.isAfter(threshold);
      case ">=" -> targetDate.isAfter(threshold) || targetDate.equals(threshold);
      case "=" -> targetDate.isEqual(threshold);
      default ->
          throw new IllegalStateException("Unexpected filter comparator: " + filter.comparator());
    };
  }

  private CodeableConcept initServiceRequestCode() {
    return new CodeableConcept()
        .setCoding(List.of(new Coding().setSystem("http://snomed.info/sct").setCode("59615004")));
  }
  ...
  protected Reference getObservationsReference(Observation obs) {
    return new Reference(
        getConditionalReference(ResourceType.Observation, obs.getIdentifierFirstRep()));
  }

  protected void addResourceToBundle(
      Bundle bundle, DomainResource resource, Identifier identifier) {
    ...
    bundle
        ...
        .setRequest(
            new Bundle.BundleEntryRequestComponent()
                ...
                .setUrl(getConditionalReference(resource.getResourceType(), identifier)));
  }

  protected String getConditionalReference(ResourceType resourceType, String identifierValue) {
    ...
    switch (resourceType) {
      ...
      case ServiceRequest -> idSystem = fhirProperties().getSystems().getServiceRequestId();
      case DiagnosticReport -> idSystem = fhirProperties().getSystems().getDiagnosticReportId();
    ...
    return getConditionalReference(
        resourceType, new Identifier().setSystem(idSystem).setValue(identifierValue));
  }

  protected String getConditionalReference(ResourceType resourceType, Identifier identifier) {

    return String.format(
        "%s?identifier=%s|%s", resourceType, identifier.getSystem(), identifier.getValue());
  }

  protected String sanitizeId(String id) {
    // replace invalid characters with "-"
    var replaced = id.replaceAll("[^A-Za-z0-9\\-\\.]", "-");
    // max 64 characters
    return replaced.substring(0, Math.min(replaced.length(), MAX_IDENTIFIER_LENGTH));
  }
  ...
  protected String createId(Identifier identifier, DateTimeType dateTimeType) {
    return createId(identifier.getSystem(), identifier.getValue(), dateTimeType);
  }

  protected String createId(String idSystem, String id, DateTimeType dateTimeType) {
    return TimestampPrefixedId.createNewIdentityValue(
        dateTimeType, hasher().apply(idSystem + "|" + id));
  }

  protected <TR extends Resource> List<TR> getBundleEntryResources(
      Bundle bundle, Class<TR> domainType) {
  ...
  protected void setPatient(String patientId, Bundle bundle) {
    ...
    getBundleEntryResources(bundle, ServiceRequest.class)
        .forEach(
            r ->
                r.getSubject()
                    .setReference(getConditionalReference(ResourceType.Patient, patientId)));
    getBundleEntryResources(bundle, DiagnosticReport.class)
        .forEach(
            r ->
                r.getSubject()
                    .setReference(getConditionalReference(ResourceType.Patient, patientId)));
    getBundleEntryResources(bundle, Observation.class)
        .forEach(
            o ->
                o.getSubject()
                    .setReference(getConditionalReference(ResourceType.Patient, patientId)));
  ...
  protected Observation.ObservationReferenceRangeComponent parseReferenceRange(
      String rangeString, Type value) {

    if (rangeString == null) {
      return null;
    }

    var refRange = new Observation.ObservationReferenceRangeComponent().setText(rangeString);
